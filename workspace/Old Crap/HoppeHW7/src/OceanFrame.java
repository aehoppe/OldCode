/**
 * Class OceanFrame provides the user interface for a simple simulation
 * program.
 * 
...
public class OceanFrame extends JFrame {
    private OceanModel myModel;
    private OceanPanel myPicture;
    ...
    public OceanFrame(int width, int height) {
        ...
        setTitle("CSC142 ****OCEAN**** simulation");
        setDefaultCloseOperation(EXIT_ON_CLOSE);
        myModel = new OceanModel(width, height);

        // set up critter picture panel
        myPicture = new OceanPanel(myModel);
    ...
    public void start() {
        ...
        if (myModel.getCounts().isEmpty()) {
            System.out.println("nothing to simulate--no animals");
            return;
        }
        started = true;
        addClassCounts();
    ...
    public void add(int number, Class<? extends SeaCreature> c) {
